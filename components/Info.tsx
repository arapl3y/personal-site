const Info = () => {
  return (
    <section className="text-off-black dark:text-off-white mt-12 md:mt-24 flex w-full max-w-4xl flex-col gap-4">
      <p>
        Software Engineer based on Gadigal land in Australia — developing
        products that improve people&apos;s everyday lives.
      </p>

      <div>
        <p>
          Experience working on large, small, production and prototype projects
          for organisations across Government, Education, Finance, Environment
          and Health.
        </p>

        <p className="my-4">Specialising in:</p>
        <ul className="ml-2 flex flex-col">
          <li>⭢ Development</li>
          <li>⭢ Design</li>
          <li>⭢ Motion</li>
        </ul>
      </div>
    </section>
  );
};

export default Info;
